"""Server-side persistent audio cache.
...
# Default cache directory (persistent, separate from temp files)
CACHE_DIR = os.environ.get("TTS_CACHE_DIR", "/app/api/audio_cache")
...
def _cache_key(text: str, voice: str, model: str, speed: float, fmt: str) -> str:
    """Deterministic hash of synthesis parameters."""
    raw = f"{text}|{voice}|{model}|{speed:.2f}|{fmt}"
    return hashlib.sha256(raw.encode("utf-8")).hexdigest()


def _cache_path(key: str, fmt: str) -> str:
    """Return the file path for a cache entry (2-level subdirectory to avoid huge flat dirs)."""
    return os.path.join(CACHE_DIR, key[:2], key[2:4], f"{key}.{fmt}")


async def get_cached(
    text: str, voice: str, model: str, speed: float, fmt: str
) -> Optional[str]:
    """Return path to cached audio file if it exists, else None."""
    key = _cache_key(text, voice, model, speed, fmt)
...
async def put_cached(
    text: str, voice: str, model: str, speed: float, fmt: str, data: bytes
) -> str:
